InModuleScope "ServiceNow" {
    Describe "Update-ServiceNowIncident" -Tag @('Unit') {
        BeforeAll {
        ...
        Context "Calling with no script scoped Auth set and no explicity credentials passed" {
            It "Should Throw if Script Scoped Auth not set and no explicit Credentials Passed" {
                {Update-ServiceNowIncident -SysId 'ABC' -Values @{assignee = 'user1'}} | Should -Throw
            }
        }
        Context "Update-ServiceNowIncident - Should Not Throw" {
            BeforeAll {
            ...
            It "Update-ServiceNowIncident - Should Not Throw" {
                {Update-ServiceNowIncident -SysId 'ABC' -Values @{assignee = 'user1'}} | Should -Not -Throw
        ...
        Context "Update-ServiceNowIncident - Script Scoped PSCredential Set" {
            BeforeAll {
                $Script:ConnectionObj = @{
                    uri        = 'dev123.service-now.com'
