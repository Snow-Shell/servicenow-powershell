function New-ServiceNowRequest {
    <#
    .SYNOPSIS
        Generates a new ServiceNow request
    
    .DESCRIPTION
        Generates a new ServiceNow request using predefined or custom fields by invoking the ServiceNow API
    
    .PARAMETER Caller
        sys_id of the caller of the request (user Get-ServiceNowUser to retrieve this)
    
    .PARAMETER ShortDescription
        Short description of the request
    
    .PARAMETER Description
       Long description of the request
    ...
    .PARAMETER Category
        Category of the request (e.g. 'Network')
    
    .PARAMETER Subcategory
        Subcategory of the request (e.g. 'Network')
    
    .PARAMETER ConfigurationItem
        sys_id of the configuration item of the request
    ...
    .EXAMPLE
        Generate a basic request attributed to the caller "UserName" with descriptions, categories, assignment groups and CMDB items set.
    
        New-ServiceNowrequest -Caller UserName -ShortDescription 'New PS request' -Description 'This request was created from Powershell' -AssignmentGroup ServiceDesk -Comment 'Inline Comment' -Category Office -Subcategory Outlook -ConfigurationItem UserPC1
    
    .EXAMPLE
        Generate an Request by 'splatting' all fields used in the 1st example plus some additional custom ServiceNow fields (These must exist in your ServiceNow instance),  This example uses the caller's sys_id value for identification.
    
        $newServiceNowRequestSplat = @{
            Caller            = '55ccf91161924edc979d8e7e5627a47d'
            ShortDescription  = 'New PS Request'
            Description       = 'This request was created from Powershell'
        ...
        New-ServiceNowRequest @newServiceNowRequestSplat
     #>
